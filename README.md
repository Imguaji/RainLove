...
- 保存指定 BV 号；可在打开哔哩哔哩后延迟发送播放命令，后台或锁屏时通过通知启动链路
...
哔哩哔哩模式接受 BV 号或包含 BV 号的完整视频链接。启用“打开后尝试自动播放”时，RainLove 会打开哔哩哔哩 App，并在播放器加载期间发送三次播放命令。该行为会受哔哩哔哩版本、网络和页面状态影响。由于 Android 后台启动限制，后台或锁屏触发时需要点击通知才能启动链路；外部播放器启动后，RainLove 不会在心率恢复时强制关闭它。
