...
- 可保存多套命名触发方案，在停止监测后切换阈值、设备和播放目标
- 自定义触发心率和恢复心率
...
在“触发方案”中输入名称可保存或覆盖当前配置，停止监测后点击方案名即可切换。方案包含阈值、保持时间、Demo/设备类型与所选设备、播放目标和媒体链接；“开机自动恢复监测”是全局选项，不随方案切换。本地音乐方案依赖先前授予的文件读取权限，文件移动或权限失效时需要重新选择。
